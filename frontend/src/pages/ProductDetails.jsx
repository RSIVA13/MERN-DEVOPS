...
const ProductDetails = () => {
    ...
    useEffect(()=>{
        if(reviewSuccess){
            toast.success('Review Submitted Successful',{position:'top-center',autoClose:3000})
            setUserRating(0);
            setComment('');
    ...
    <div className="product-detail-container">
        ...
        <div className="product-image-container">
            ...
            {product.image.length>1 && (
                ...
                <img src={img.url} alt={`thumbnail ${index+1}`} onClick={()=>setSelectedImage(img.url)} className='thumbnail-image' />
                ))}
            </div>
            )}
        </div>
        ...
        <div className="product-info">
            ...
            <form  className="review-form" onSubmit={handleReviewSubmit}>
                ...
                <textarea placeholder='Write Your review here...' value={comment} onChange={(e)=>setComment(e.target.value)} className='review-input' required></textarea>
                <button className='submit-review-btn' disabled={reviewLoading}>{reviewLoading ? 'Review Submitting...':'Submit Review'}</button>
            </form>
        </div>
    </div>
